...
repeatBtn.addEventListener("click", ()=>{
    ...
    let getText = repeatBtn.innerText //getting this tag innerText of icon
...
//let's create li according to the array length for list
for(let i=0; i<allMusic.length; i++){
    //let's pass the song name, artist from the array to
    ...
    liAudioTag.addEventListener("loadeddata", ()=>{
        ...
        if(totalSec < 10){ //if sec is less than 10 
            totalSec = `0${totalSec}`;
        }
...
function playingNow(){
    for( let j=0; j<allLiTags.length; j++){
        ...
        //remove playing class from all other li expect the last one which is class
